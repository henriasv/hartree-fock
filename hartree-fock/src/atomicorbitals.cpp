...
AtomicOrbitals::AtomicOrbitals(int _numParticles, int _nuclearCharge, int _numBasisFunctions)
{
    numParticles = _numParticles;
    if ((numParticles % 2) != 0) {
        std::cout << "Unsupported particle number " << numParticles << std::endl;
        exit(1);
    }
    numK = numParticles/2;
    numBasisFunctions = _numBasisFunctions;
    Z = _nuclearCharge;
    Q = createQ(Z);
}

double AtomicOrbitals::overlapIntegral(int p, int q)
{
    if (p == q)
        return 1.0;
    else
...
double AtomicOrbitals::coupledIntegral(int p, int q, int r, int s)
{
    return Q(p, r)(q, s);
}

double AtomicOrbitals::uncoupledIntegral(int p, int q)
{
    if (p == q) {
        return -Z*Z/(2.0*(p+1.0)*(p+1.0));
    }
    else
    {
        return 0;
...
std::string AtomicOrbitals::str()
{
    return std::string("Atomic Orbitals");
} 

arma::field<arma::mat> AtomicOrbitals::createQ(int X)
{
